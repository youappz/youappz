...
export default async (client: Client): Promise<number> => {
  ...
  if (status === 'not_linked') {
    ...
    // even though the `appz.json` file can change
  ...
  // Merge dotenv config, `env` from appz.json, and `--env` / `-e` arguments
  ...
  // Merge build env out of  `build.env` from appz.json, and `--build-env` args
  ...
  try {
    const createArgs: any = {
      ...
      nowConfig: {
        ...localConfig,
        // `images` is allowed in "appz.json" and processed
...
function handleCreateDeployError(
  ...
  if (error instanceof SchemaValidationFailed) {
    ...
    const fileName = localConfig[fileNameSymbol] || 'appz.json';
