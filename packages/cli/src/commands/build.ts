...
import { APPZ_DIR } from '../util/projects/link';
...
export default async function main(client: Client): Promise<number> {
  ...
  let project = await readProjectSettings(join(cwd, APPZ_DIR));
  ...
  while (!project?.settings) {
    ...
    project = await readProjectSettings(join(cwd, APPZ_DIR));
  }
  ...
  try {
    const envPath = join(cwd, APPZ_DIR, `.env.${target}.local`);
...
async function doBuild(
  ...
  const [pkg, vercelConfig] = await Promise.all([
    ...
    readJSONFile<VercelConfig>(join(workPath, 'appz.json')),
  ]);
  ...
  if (vercelConfig) {
    vercelConfig[fileNameSymbol] = 'appz.json';
  }
