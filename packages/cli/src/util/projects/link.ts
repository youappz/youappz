...
export async function getLinkFromDir<T = ProjectLink>(
  ...
  try {
    const json = await readFile(join(dir, APPZ_DIR_PROJECT), 'utf8');
...
export async function linkFolderToProject(
  ...
  try {
    await ensureDir(join(path, APPZ_DIR));
  } catch (err: unknown) {
  ...
  await writeFile(
    join(path, APPZ_DIR, APPZ_DIR_PROJECT),
    JSON.stringify(projectLink)
  );

  await writeFile(
    join(path, APPZ_DIR, APPZ_DIR_README),
    await readFile(join(__dirname, 'APPZ_DIR_README.txt'), 'utf8')
  );
  ...
  try {
    ...
    if (!gitIgnore.split(EOL).includes(APPZ_DIR)) {
      gitIgnore += `${
