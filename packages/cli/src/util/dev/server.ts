...
export default class DevServer {
  ...
  async _getVercelConfig(): Promise<VercelConfig> {
    ...
    const [
      pkg = null,
      // The default empty `appz.json` is used to serve all
      // files as static when no `appz.json` is present
      vercelConfig = { version: 2, [fileNameSymbol]: 'appz.json' },
    ] = await Promise.all([
  ...
  validateEnvConfig(type: string, env: Env = {}, localEnv: Env = {}): Env {
    // Validate if there are any missing env vars defined in `appz.json`,
