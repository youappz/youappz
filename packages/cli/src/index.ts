...
const APPZ_DIR = getGlobalPathConfig();
const APPZ_CONFIG_PATH = configFiles.getConfigFilePath();
const APPZ_AUTH_CONFIG_PATH = configFiles.getAuthConfigFilePath();
...
Sentry.init({
  dsn: SENTRY_DSN,
  release: `youappz-cli@${pkg.version}`,
  environment: isCanary ? 'canary' : 'stable',
});

let client: Client;
let debug: (s: string) => void = () => {};
let apiUrl = 'https://api.youappz.com';
...
const main = async () => {
  ...
  if (notifier.update && notifier.update.latest !== pkg.version && isTTY) {
    ...
    console.log(
      info(
        `Changelog: https://github.com/youappz/youappz/releases/tag/vercel@${latest}`
      )
    );
  }
  ...
  if (betaCommands.includes(targetOrSubcommand)) {
    console.log(
      `${chalk.grey(
        `${getTitleName()} CLI ${
          pkg.version
        } ${targetOrSubcommand} (beta) — https://youappz.com/feedback`
      )}`
    );
  } else {
    output.print(
      `${chalk.grey(
        `${getTitleName()} CLI ${pkg.version}${
          isCanary ? ' — https://youappz.com/feedback' : ''
        }`
      )}\n`
    );
  }
  ...
  if (
    ...
    if (isTTY) {
      ...
      output.debug(`Saved credentials in "${hp(APPZ_DIR)}"`);
    } else {
      output.prettyError({
        message:
  ...
  try {
    ...
    if (isErrnoException(err) && err.code === 'ENOTFOUND') {
      ...
      // "request to https://api.youappz.com/v2/user failed, reason: getaddrinfo ENOTFOUND api.youappz.com"
